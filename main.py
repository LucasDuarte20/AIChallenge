...
class CreateAgendaWindow(ctk.CTkToplevel):
    ...
    def _build(self) -> None:
        ...
        # Filtros de Visita
        self._section_label(lp, "Filtros")
        
        filt_f = ctk.CTkFrame(lp, fg_color="transparent")
        filt_f.pack(fill="x", pady=(2, 10))
        
        self._chk_empresas = ctk.CTkCheckBox(filt_f, text="Empresas", font=(FONT, 12), fg_color=RED, hover_color=RED_HOVER, width=100)
        self._chk_empresas.grid(row=0, column=0, pady=2, sticky="w")
        self._chk_empresas.select()
        
        self._chk_personas = ctk.CTkCheckBox(filt_f, text="Personas", font=(FONT, 12), fg_color=RED, hover_color=RED_HOVER, width=100)
        self._chk_personas.grid(row=0, column=1, pady=2, sticky="w")
        self._chk_personas.select()

        self._chk_mvd = ctk.CTkCheckBox(filt_f, text="MVD", font=(FONT, 12), fg_color=RED, hover_color=RED_HOVER, width=100)
        self._chk_mvd.grid(row=1, column=0, pady=4, sticky="w")
        self._chk_mvd.select()

        self._chk_interior = ctk.CTkCheckBox(filt_f, text="Interior", font=(FONT, 12), fg_color=RED, hover_color=RED_HOVER, width=100)
        self._chk_interior.grid(row=1, column=1, pady=4, sticky="w")
        self._chk_interior.select()
    ...
    def _generate(self) -> None:
        ...
        inc_emp = bool(self._chk_empresas.get())
        inc_per = bool(self._chk_personas.get())
        inc_mvd = bool(self._chk_mvd.get())
        inc_int = bool(self._chk_interior.get())
        ...
        try:
            ...
            visits, first_next = build_agenda(
                ...
                visits_per_day=vpd,
                include_empresas=inc_emp,
                include_personas=inc_per,
                include_mvd=inc_mvd,
                include_interior=inc_int,
            )
